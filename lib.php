...
/**
 * Starter theme background callbacks.
 *
 ...
 * @return string
 */
function theme_starter_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_starter', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_starter and not theme_boost (see the line above).
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/starter/scss/_settings.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/starter/scss/starter.scss');
 ...
 * @param $settingname
 */
function theme_starter_update_settings_images($settingname) {
    global $CFG;

    // The setting name that was updated comes as a string like 's_theme_starter_loginbackgroundimage'.
    ...
    $pathname = $CFG->dataroot . '/pix_plugins/theme/starter/' . $settingname . '.' . $extension;

    // This pattern matches any previous files with maybe different file extensions.
    $pathpattern = $CFG->dataroot . '/pix_plugins/theme/starter/' . $settingname . '.*';

    // Make sure this dir exists.
    @mkdir($CFG->dataroot . '/pix_plugins/theme/starter/', $CFG->directorypermissions, true);
